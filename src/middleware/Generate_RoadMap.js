...
//   const prompt = `
//     Generate a learning roadmap for ${topic} with 6-8 steps. For each step, include:
...
//     Example:
//     "Step 1: HTML & CSS | Description: HTML (Hypertext Markup Language) is the standard markup language for web pages. CSS styles the layout and design. | Depends on: None"
//     "Step 8: Deployment & DevOps | Description: CI/CD, Docker, and Kubernetes streamline app deployment. They ensure scalability and reliability. | Depends on: Step 5, Step 7"
//     Do not include ** or titles, only the steps.
//   `;
...
const generateRoadmap = async ({ topic, level, duration }) => {
  ...
  const levelDesc = levelMap[level] || levelMap['1']; // Mặc định trung cấp
  const durationDesc = durationMap[duration] || durationMap['1']; // Mặc định 3 tháng
